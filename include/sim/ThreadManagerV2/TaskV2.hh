...
class TaskV2 : public Task {
	friend std::ostream& operator<<(std::ostream& os, const TaskV2& t);

public:
	TaskV2() : next_execution_cycle(0), id(0) { ; }
	~TaskV2() { ; }

	// Constructor to initialize the task
	TaskV2(SimBase* base, std::string name) {
	...
	bool operator<(const TaskV2& other) const { return next_execution_cycle < other.next_execution_cycle; }
	bool operator==(const TaskV2& other) const { return next_execution_cycle == other.next_execution_cycle; }
...
inline std::ostream& operator<<(std::ostream& os, const TaskV2& t) {
	std::stringstream ss;
