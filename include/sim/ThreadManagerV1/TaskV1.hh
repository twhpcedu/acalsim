...
class TaskV1 : public Task {
	friend std::ostream& operator<<(std::ostream& os, const TaskV1& t);

public:
	TaskV1() : next_execution_cycle(0), id(0) { ; }
	~TaskV1() { ; }

	// Constructor to initialize the task
	TaskV1(SimBase* base, std::string name) {
	...
	bool operator<(const TaskV1& other) const { return next_execution_cycle < other.next_execution_cycle; }
	bool operator==(const TaskV1& other) const { return next_execution_cycle == other.next_execution_cycle; }
...
inline std::ostream& operator<<(std::ostream& os, const TaskV1& t) {
	std::stringstream ss;
