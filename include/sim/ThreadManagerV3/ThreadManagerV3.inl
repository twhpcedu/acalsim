...
ThreadManagerV3<TClass>::ThreadManagerV3(const std::string& _name, unsigned int _nThreads, bool _nThreadsAdjustable)
    : TClass(_name, _nThreads, _nThreadsAdjustable) {
	this->stepTaskList                 = std::make_shared<typename TaskManagerV3<TClass>::TaskList>(1);
	this->interIterationUpdateTaskList = std::make_shared<typename TaskManagerV3<TClass>::TaskList>(1);
}

template <typename TClass>
void ThreadManagerV3<TClass>::preSimInit() {
	// Note: The following operations prepare ThreadManagerV3<TClass>::interIterationUpdateTaskList
	// for ThreadManagerV3<TClass>::runInterIterationUpdate(). However, due to performance
...
template <typename TClass>
void ThreadManagerV3<TClass>::startPhase1() {
...
template <typename TClass>
void ThreadManagerV3<TClass>::startPhase2() {
...
template <typename TClass>
void ThreadManagerV3<TClass>::finishPhase2() {
	// In Phase 2, all simulators are paused and do nothing
	// The control thread (SimTop) does all the bookkeeping things
...
template <typename TClass>
void ThreadManagerV3<TClass>::runInterIterationUpdate() {
	// Note: The most intuitive way to execute SimBase::interIterationUpdate() for all simulators
	// would be to submit ThreadManagerV3<TClass>::interIterationUpdateTaskList to TaskManagerV3.
	...
	// of dynamic thread allocation in TaskManagerV3<TClass>::scheduler().
	...
	// this->interIterationUpdateTaskList->wait();
}

template <typename TClass>
void ThreadManagerV3<TClass>::terminateAllThreads() {
	this->running = false;
	this->getTaskManager()->runnerCv.notify_all();
...
#ifdef ACALSIM_STATISTICS
template <typename TClass>
void ThreadManagerV3<TClass>::printSchedulingOverheads(double _total_time) const {
	double taskq_lock_cost_us     = NamedTimer<"TaskManagerV3-TaskQueue-Phase1">::getTimerVal();
	double runner_cv_lock_cost_us = NamedTimer<"TaskManagerV3-TaskAvailCv-Phase1">::getTimerVal();

	LABELED_STATISTICS("ThreadManagerV3")
