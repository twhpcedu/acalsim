...
class TaskV6 : public Task {
	friend std::ostream& operator<<(std::ostream& os, const TaskV6& t);

public:
	TaskV6() : next_execution_cycle(0), id(0) { ; }
	~TaskV6() { ; }

	// Constructor to initialize the task
	TaskV6(SimBase* base, std::string name) {
	...
	bool operator<(const TaskV6& other) const { return next_execution_cycle < other.next_execution_cycle; }
	bool operator==(const TaskV6& other) const { return next_execution_cycle == other.next_execution_cycle; }
...
inline std::ostream& operator<<(std::ostream& os, const TaskV6& t) {
	std::stringstream ss;
