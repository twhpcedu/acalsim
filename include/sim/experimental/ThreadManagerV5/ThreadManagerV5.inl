...
template <typename T>
void ThreadManagerV5<T>::runInterIterationUpdate() {
...
template <typename T>
void ThreadManagerV5<T>::terminateAllThreads() {
...
template <typename T>
void ThreadManagerV5<T>::startPhase1() {
...
template <typename T>
void ThreadManagerV5<T>::finishPhase1() {
	// In Phase 1, all simulators will execute one iteration
	// The control thread (SimTop) executes the SimTop::control_thread_step() function
...
template <typename T>
void ThreadManagerV5<T>::startPhase2() {
...
template <typename T>
void ThreadManagerV5<T>::finishPhase2() {
	// In Phase 2, all simulators are paused and do nothing
	// The control thread (SimTop) does all the bookkeeping things
...
#ifdef ACALSIM_STATISTICS
template <typename T>
void ThreadManagerV5<T>::printSchedulingOverheads(double _total_time) const {
	double task_avail_lock_cost_us = NamedTimer<"TaskManagerV5-TaskAvailCv-Phase1">::getTimerVal();

	LABELED_STATISTICS("ThreadManagerV5")
	    << "Scheduling Overheads: "
	    << "(1) Task Queue Unique Lock: "
	    << this->getTaskManager()->taskQueue.getUniqueLockWaitingTime() / this->nThreads / _total_time * 100 << "% "
	    << "(2) Task Queue Shared Lock: "
	    << this->getTaskManager()->taskQueue.getSharedLockWaitingTime() / this->nThreads / _total_time * 100 << "% "
	    << "(3) Task Queue Manipulation: "
	    << this->getTaskManager()->taskQueue.getTqManipTime() / this->nThreads / _total_time * 100 << "% "
	    << "(4) Task Avail Lock: " << task_avail_lock_cost_us / this->nThreads / _total_time * 100 << "%.";

	LABELED_STATISTICS("ThreadManagerV5")
	    << "Scheduling Overheads (per thread): "
	    << "(1) Task Queue Unique Lock: "
	    << this->getTaskManager()->taskQueue.getUniqueLockWaitingTime() / this->nThreads << " us, "
	    << "(2) Task Queue Shared Lock: "
	    << this->getTaskManager()->taskQueue.getSharedLockWaitingTime() / this->nThreads << " us, "
	    << "(3) Task Queue Manipulation: " << this->getTaskManager()->taskQueue.getTqManipTime() / this->nThreads
	    << " us, "
