...
template <typename T>
class ThreadManagerV8;

template <typename TFriend>
class TaskManagerV8 : public TaskManager {
	friend class ThreadManagerV8<TFriend>;
...
public:
	 ...
	 */
	class TaskList {
		friend class TaskManagerV8<TFriend>;
...
public:
	TaskManagerV8(std::string _name) : TaskManager(_name) {}
	~TaskManagerV8();
	...
	/**
	 * @brief Add a Task object to the TaskManagerV8.
	...
	ThreadManagerV8<TFriend>* getThreadManager() const override {
		return dynamic_cast<ThreadManagerV8<TFriend>*>(this->threadManager);
	}
	...
	void wait() { this->producerBarrierPtr->arrive_and_wait(); }
...
protected:
	typename TaskList::SharedPtr currTaskList;
	std::atomic<size_t>          finishedCnt = 0;

	std::atomic<bool>             consumerBarrierAvail = false;
	std::barrier<void (*)(void)>* consumerBarrierPtr   = nullptr;
	std::barrier<void (*)(void)>* producerBarrierPtr   = nullptr;
...
#include "sim/experimental/ThreadManagerV8/TaskManagerV8.inl"
