...
public:
	...
	uint8_t getPacketType() const { return (header >> 8) & 0xFF; }

	// Set packet type in header
	void setPacketType(uint8_t type) { header = (header & 0x00FF) | (type << 8); }

	// Get total work-items
	uint64_t getTotalWorkitems() const {
		return static_cast<uint64_t>(grid_size_x) * static_cast<uint64_t>(grid_size_y) *
...
public:
	...
	HSASignalEvent()
	    : Event(), signal_handle(0), signal_value(0), operation(Operation::STORE), timestamp(0), dispatch_id(0) {}

	HSASignalEvent(uint64_t handle, int64_t value, Operation op = Operation::STORE)
	    : Event(), signal_handle(handle), signal_value(value), operation(op), timestamp(0), dispatch_id(0) {}
...
public:
	...
	HSADoorbellEvent() : Event(), queue_id(0), doorbell_value(0), timestamp(0) {}

	HSADoorbellEvent(uint32_t qid, uint64_t value) : Event(), queue_id(qid), doorbell_value(value), timestamp(0) {}
...
public:
	enum class Type { READ, WRITE, COPY } type;
	...
	HSAMemoryEvent() : Event(), type(Type::READ), address(0), size(0), data(nullptr), is_complete(false) {}
