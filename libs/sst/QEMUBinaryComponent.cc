...
#include "sst/QEMUBinaryComponent.hh"
...
QEMUBinaryComponent::QEMUBinaryComponent(SST::ComponentId_t id, SST::Params& params)
	...
	tc_ =
	    registerClock(clock_freq, new SST::Clock::Handler<QEMUBinaryComponent>(this, &QEMUBinaryComponent::clockTick));
	...
	if (use_multi_device_) {
		...
		for (int i = 0; i < num_devices_; i++) {
			...
			dev.link              = configureLink(port_name, new SST::Event::Handler<QEMUBinaryComponent>(
                                                    this, &QEMUBinaryComponent::handleDeviceResponse));

			if (!dev.link) { out_.fatal(CALL_INFO, -1, "Error: Failed to configure %s\n", port_name.c_str()); }

			devices_.push_back(dev);

			out_.verbose(CALL_INFO, 1, 0, "  Device %d (%s): [0x%016" PRIx64 ", 0x%016" PRIx64 ")\n", i,
			             dev.name.c_str(), dev.base_addr, dev.base_addr + dev.size);
		}
	} else {
		// Legacy single device mode: Configure single device_port
		device_link_ = configureLink("device_port", new SST::Event::Handler<QEMUBinaryComponent>(
		                                                this, &QEMUBinaryComponent::handleDeviceResponse));
...
QEMUBinaryComponent::~QEMUBinaryComponent() {
	...
	for (auto& dev : devices_) {
		if (dev.client_fd >= 0) { close(dev.client_fd); }
		if (dev.server_fd >= 0) { close(dev.server_fd); }
		if (!dev.socket_path.empty()) { unlink(dev.socket_path.c_str()); }
	}
}
...
bool QEMUBinaryComponent::clockTick(SST::Cycle_t cycle) {
	...
	if (state_ == QEMUState::RUNNING || state_ == QEMUState::WAITING_DEVICE) {
		...
		if (use_multi_device_) {
			...
			if (socket_ready_ && client_fd_ >= 0) { handleMMIORequest(); }
		}
	}
...
void QEMUBinaryComponent::launchQEMU() {
	...
	if (use_multi_device_) {
		...
		for (size_t i = 0; i < devices_.size(); i++) { setupDeviceSocket(&devices_[i], i); }
	} else {
		// Legacy single-device mode
		...
		if (server_fd_ < 0) { out_.fatal(CALL_INFO, -1, "Error: Failed to create server socket\n"); }
	...
	if (qemu_pid_ == 0) {
		...
		if (use_multi_device_) {
			for (auto& dev : devices_) { close(dev.server_fd); }
		} else {
			close(server_fd_);
		}
...
void QEMUBinaryComponent::sendDeviceRequest(uint8_t type, uint64_t addr, uint64_t data, uint8_t size) {
	if (use_multi_device_) {
		...
		out_.verbose(CALL_INFO, 2, 0,
		             "Sending device request: type=%d addr=0x%016" PRIx64 " data=0x%08x size=%u req_id=%" PRIu64 "\n",
...
DeviceInfo* QEMUBinaryComponent::findDeviceForAddress(uint64_t address) {
	for (auto& dev : devices_) {
		if (address >= dev.base_addr && address < dev.base_addr + dev.size) { return &dev; }
	}
	return nullptr;
}
...
void QEMUBinaryComponent::routeToDevice(DeviceInfo* device, uint8_t type, uint64_t addr, uint64_t data, uint8_t size) {
	...
	auto* event =
	    new MemoryTransactionEvent(tx_type, addr, static_cast<uint32_t>(data), static_cast<uint32_t>(size), req_id);

	out_.verbose(CALL_INFO, 2, 0,
	             "Routing to %s: type=%d addr=0x%016" PRIx64 " data=0x%08x size=%u req_id=%" PRIu64 "\n",
...
void QEMUBinaryComponent::pollDeviceSockets() {
	for (auto& dev : devices_) {
		...
		if (!dev.socket_ready) { acceptDeviceConnection(&dev); }

		// Poll for MMIO requests if connected
		if (dev.socket_ready && dev.client_fd >= 0) { handleMMIORequest(&dev); }
	}
}

void QEMUBinaryComponent::handleMMIORequest(DeviceInfo* device) {
	if (!device->socket_ready || device->client_fd < 0) { return; }
