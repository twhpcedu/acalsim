...
ACALSimMMIODevice::ACALSimMMIODevice(SST::ComponentId_t id, SST::Params& params) :
	...
	cpu_link_ = configureLink("cpu_port",
	    new SST::Event::Handler<ACALSimMMIODevice>(this, &ACALSimMMIODevice::handleMemoryTransaction));
	if (!cpu_link_) {
		output_.fatal(CALL_INFO, -1, "Error: Failed to configure cpu_port\n");
	}

	// Configure IRQ link (for interrupt signaling)
	irq_link_ = configureLink("irq_port");
	if (!irq_link_) {
		output_.fatal(CALL_INFO, -1, "Error: Failed to configure irq_port\n");
	}
	...
	output_.verbose(CALL_INFO, 1, 0,
	    "ACALSim MMIO Device initialized: base=0x%lx, size=%lu, irq=%u, latency=%u\n",
...
void ACALSimMMIODevice::setup() {
	output_.verbose(CALL_INFO, 1, 0, "ACALSim MMIO Device setup complete\n");
}

void ACALSimMMIODevice::finish() {
	output_.verbose(CALL_INFO, 1, 0,
	    "ACALSim MMIO Device finish: MMIO ops: R=%lu W=%lu, Operations=%lu, IRQs=%lu\n",
	    stat_mmio_reads_->getCollectionCount(),
	    stat_mmio_writes_->getCollectionCount(),
	    total_ops_, stat_interrupts_->getCollectionCount());
}
...
void ACALSimMMIODevice::handleMemoryTransaction(SST::Event* ev) {
	...
	if (!trans) {
		output_.fatal(CALL_INFO, -1, "Error: Received non-transaction event on cpu_port\n");
	}
	...
	if (is_load) {
		...
		output_.verbose(CALL_INFO, 2, 0,
		    "MMIO READ: offset=0x%lx, data=0x%x, size=%u, req_id=%lu\n",
		    offset, resp_data, size, req_id);
	} else {
		// MMIO Write
		...
		output_.verbose(CALL_INFO, 2, 0,
		    "MMIO WRITE: offset=0x%lx, data=0x%x, size=%u, req_id=%lu\n",
		    offset, data, size, req_id);
	}
...
uint32_t ACALSimMMIODevice::readRegister(uint64_t offset) {
	switch (offset) {
		...
		case REG_INT_STATUS:
			return reg_int_status_;

		case REG_INT_ENABLE:
			return reg_int_enable_;

		case REG_SRC_ADDR:
			return reg_src_addr_;

		case REG_DST_ADDR:
			return reg_dst_addr_;
		...
		case REG_DATA_OUT:
			return reg_data_out_;

		case REG_CYCLE_COUNT:
			return static_cast<uint32_t>(cycle_count_);

		default:
			output_.verbose(CALL_INFO, 3, 0, "Read from unknown register: offset=0x%lx\n", offset);
			return 0;
	}
}
...
void ACALSimMMIODevice::startOperation() {
	...
	output_.verbose(CALL_INFO, 1, 0,
	    "Operation started: src=0x%x, dst=0x%x, len=%u, latency=%lu cycles\n",
...
void ACALSimMMIODevice::completeOperation() {
	...
	output_.verbose(CALL_INFO, 1, 0,
	    "Operation completed: latency=%lu cycles, total_ops=%lu\n",
	    latency, total_ops_);
...
void ACALSimMMIODevice::generateInterrupt(uint32_t irq_bits) {
	...
	output_.verbose(CALL_INFO, 2, 0,
	    "Interrupt generated: bits=0x%x, int_status=0x%x\n",
...
void ACALSimMMIODevice::clearInterrupt(uint32_t irq_bits) {
	...
	output_.verbose(CALL_INFO, 2, 0,
	    "Interrupt cleared: bits=0x%x, int_status=0x%x\n",
	    irq_bits, reg_int_status_);
...
void ACALSimMMIODevice::updateInterruptLine() {
	...
	if (should_assert && !irq_asserted_) {
		...
		output_.verbose(CALL_INFO, 2, 0,
		    "IRQ %u ASSERTED: int_status=0x%x, int_enable=0x%x\n",
		    irq_num_, reg_int_status_, reg_int_enable_);
		...
		output_.verbose(CALL_INFO, 2, 0,
		    "IRQ %u DEASSERTED: int_status=0x%x, int_enable=0x%x\n",
		    irq_num_, reg_int_status_, reg_int_enable_);
	}
}
