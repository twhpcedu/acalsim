...
HSAHostComponent::HSAHostComponent(SST::ComponentId_t id, SST::Params& params)
	...
	signal_port_ = configureLink(
	    "signal_port", new SST::Event::Handler<HSAHostComponent>(this, &HSAHostComponent::handleCompletionSignal));
	if (!signal_port_) { output_.fatal(CALL_INFO, -1, "Error: Failed to configure signal_port\n"); }

	doorbell_port_ = configureLink(
	    "doorbell_port", new SST::Event::Handler<HSAHostComponent>(this, &HSAHostComponent::handleDoorbellAck));
	...
	registerClock(clock_freq, new SST::Clock::Handler<HSAHostComponent>(this, &HSAHostComponent::clockTick));

	output_.verbose(CALL_INFO, 1, 0,
	                "HSA Host Component initialized: %u dispatches, workgroup=%ux%ux%u, grid=%ux%ux%u\n",
	                num_dispatches_, workgroup_size_x_, workgroup_size_y_, workgroup_size_z_, grid_size_x_,
	                grid_size_y_, grid_size_z_);
}

HSAHostComponent::~HSAHostComponent() {}

void HSAHostComponent::setup() { output_.verbose(CALL_INFO, 1, 0, "HSA Host setup complete\n"); }

void HSAHostComponent::finish() {
	output_.verbose(CALL_INFO, 1, 0, "HSA Host finish: Submitted=%u, Completed=%u\n", dispatches_submitted_,
	                dispatches_completed_);

	if (dispatches_completed_ != dispatches_submitted_) {
		output_.output(CALL_INFO, "Warning: Not all dispatches completed (%u/%u)\n", dispatches_completed_,
		               dispatches_submitted_);
	}
}
...
void HSAHostComponent::submitKernelDispatch() {
	...
	output_.verbose(CALL_INFO, 2, 0, "Submitted dispatch %u: signal=0x%lx, workitems=%lu, workgroups=%lu\n",
...
void HSAHostComponent::handleCompletionSignal(SST::Event* ev) {
	...
	if (!signal) { output_.fatal(CALL_INFO, -1, "Error: Received non-signal event on signal port\n"); }

	uint64_t signal_handle = signal->signal_handle;
	int64_t  signal_value  = signal->signal_value;

	output_.verbose(CALL_INFO, 2, 0, "Received completion signal: handle=0x%lx, value=%ld\n", signal_handle,
	                signal_value);
	...
	if (signal_value == 0) {
		...
		if (time_it != dispatch_submit_time_.end()) {
			...
			output_.verbose(CALL_INFO, 1, 0, "Dispatch %u completed: latency=%lu ns\n", dispatch_id, latency);
		...
		if (dispatches_completed_ == num_dispatches_) {
			output_.verbose(CALL_INFO, 1, 0, "All %u dispatches completed\n", num_dispatches_);
		}
	} else {
		output_.output(CALL_INFO, "Warning: Dispatch %u signaled with error value %ld\n", dispatch_id, signal_value);
	}
...
void HSAHostComponent::handleDoorbellAck(SST::Event* ev) {
	...
	if (ack) {
		output_.verbose(CALL_INFO, 3, 0, "Doorbell acknowledged: queue=%u, value=%lu\n", ack->queue_id,
		                ack->doorbell_value);
	}
	delete ev;
}
