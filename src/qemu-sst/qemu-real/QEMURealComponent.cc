...
QEMURealComponent::QEMURealComponent(SST::ComponentId_t id, SST::Params& params)
    ...
    device_link_ = configureLink("device_port",
                                   new SST::Event::Handler<QEMURealComponent>(this, &QEMURealComponent::handleDeviceResponse));
...
void QEMURealComponent::handleDeviceResponse(SST::Event* ev) {
    ...
    out_.verbose(CALL_INFO, 2, 0, "Received device response: req_id=%lu data=0x%x success=%d\n",
                 req_id, resp->getData(), resp->getSuccess());
...
bool QEMURealComponent::parseSSTCommand(const std::string& cmd, std::string& operation, uint64_t& addr, uint32_t& data) {
...
void QEMURealComponent::sendSerialResponse(const std::string& response) {
    ...
    if (serial_fd_ >= 0) {
        write(serial_fd_, response.c_str(), response.length());
    }
}
