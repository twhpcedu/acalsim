...
bool virtio_sst_connect(VirtIOSST *s, Error **errp)
{
    ...
    if (ret < 0) {
        error_setg_errno(errp, errno, "Failed to connect to SST at %s",
                        s->socket_path);
        close(s->socket_fd);
        s->socket_fd = -1;
        return false;
...
void virtio_sst_process_request(VirtIOSST *s, VirtQueue *vq, VirtQueueElement *elem)
{
    ...
    case SST_REQ_CONFIGURE:
        ...
        if (!success) {
            resp->status = SST_STATUS_NO_DEVICE;
        }
        break;
...
void virtio_sst_realize(DeviceState *dev, Error **errp)
{
    ...
    s->req_vq = virtio_add_queue(vdev, VIRTIO_SST_QUEUE_SIZE,
                                 virtio_sst_handle_request);
    s->resp_vq = virtio_add_queue(vdev, VIRTIO_SST_QUEUE_SIZE,
                                  virtio_sst_handle_response);
    s->event_vq = virtio_add_queue(vdev, VIRTIO_SST_QUEUE_SIZE,
                                   virtio_sst_handle_event);
...
void virtio_sst_unrealize(DeviceState *dev)
{
    ...
    qemu_log("VirtIO SST: Statistics - Requests: %lu, Responses: %lu, "
             "Events: %lu, Errors: %lu\n",
             s->total_requests, s->total_responses,
             s->total_events, s->total_errors);
 ...
 * Module Registration
 */
static void virtio_sst_register_types(void)
{
    type_register_static(&virtio_sst_info);
}
