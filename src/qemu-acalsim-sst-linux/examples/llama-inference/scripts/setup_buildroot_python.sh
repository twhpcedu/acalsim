...
echo "  2. Configure for RISC-V + Python 3 + NumPy + PyTorch deps"
...
echo "Creating initramfs symlink..."
ln -sf "$ROOTFS_CPIO" "$INITRAMFS_LINK"

if [ -L "$INITRAMFS_LINK" ]; then
	echo -e "${GREEN}✓${NC} Initramfs symlink created: $INITRAMFS_LINK"
	echo "  -> $ROOTFS_CPIO"
	ROOTFS_SIZE=$(du -h "$ROOTFS_CPIO" | cut -f1)
	echo "  Size: $ROOTFS_SIZE compressed"
else
...
echo "  ✓ Initramfs: $INITRAMFS_LINK"
echo "  ✓ Boot script: run_qemu_initramfs.sh"
echo ""
...
echo "     ./run_qemu_initramfs.sh"
echo "  2. Login (typically root with no password)"
echo "  3. In Linux, verify Python:"
echo "     python3 --version"
echo "     python3 -c 'import numpy; print(numpy.__version__)'"
echo "  4. System will boot to:"
echo "     Welcome to ACAL Simulator RISC-V Linux"
echo "     acalsim-riscv login:"
echo ""
