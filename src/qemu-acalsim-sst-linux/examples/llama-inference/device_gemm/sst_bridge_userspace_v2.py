...
#!/usr/bin/env python3
"""User-space SST Bridge - v2 (uses shared folder for sockets)"""
...
class SSTBridge:
	...
	def __init__(self):
		# Use shared folder instead of /tmp for cross-Docker-QEMU communication
		self.qemu_sock = "/home/user/projects/device_gemm/sst-bridge.sock"
		self.sst_sock = "/tmp/qemu-sst-llama.sock"

	def connect_sst(self):
		if not os.path.exists(self.sst_sock):
			print(f"SST not found at {self.sst_sock}")
			return None
		try:
			s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
			s.connect(self.sst_sock)
			print(f"Connected to SST at {self.sst_sock}")
			return s
		except Exception as e:
			print(f"Failed to connect to SST: {e}")
			return None

	def run(self):
		print("=" * 60)
		print("User-Space SST Bridge v2")
		print("=" * 60)
		...
		# Fix permissions so QEMU can connect
		os.chmod(self.qemu_sock, 0o777)
		print(f"✓ Socket permissions set to 777")

		srv.listen(5)
		print(f"Listening on {self.qemu_sock}")
		print("(This socket is in the shared folder, accessible from QEMU)")
