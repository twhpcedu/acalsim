...
print(f"Total compute power: {NUM_ACCELERATORS * CORES_PER_ACCELERATOR} cores @ {ACCELERATOR_CLOCK}")
print("=" * 60)
...
hsa_host.addParams({
    "num_agents": str(NUM_ACCELERATORS),
    "verbose": "1",
    "clock": "1GHz"
})
...
for i in range(NUM_ACCELERATORS):
    print(f"HSA Compute Agent {i}: hsa_compute_{i} ({CORES_PER_ACCELERATOR} cores, {ACCELERATOR_CLOCK})")
