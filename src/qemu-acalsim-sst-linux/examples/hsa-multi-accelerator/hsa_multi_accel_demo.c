...
void *compute_kernel(void *arg) {
    ...
    printf("  [Thread %d] Submitting to device %d (rows %d-%d, %lu units)...",
           w->device_id, w->device_id, w->start_row, w->end_row - 1,
           req.payload.compute.compute_units);
    fflush(stdout);
...
int main() {
    ...
    if (write(g_sst_fd, &req, sizeof(req)) != sizeof(req) ||
        read(g_sst_fd, &resp, sizeof(resp)) != sizeof(resp)) {
