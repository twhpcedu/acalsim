 ...
 */
int sst_submit_request(struct sst_virt_device *sdev, struct SSTRequest *req,
                      struct SSTResponse *resp)
{
    ...
    /* Wait for response */
    ret = wait_event_interruptible_timeout(req_ctx->wait,
                                          req_ctx->state == SST_REQ_STATE_COMPLETED,
...
__poll_t sst_poll(struct file *filp, struct poll_table_struct *wait)
{
    ...
    if (fctx->active_req && fctx->active_req->state == SST_REQ_STATE_COMPLETED)
...
{
    sdev->vdev->config->del_vqs(sdev->vdev);
}
...
int sst_read_config(struct sst_virt_device *sdev)
{
    ...
    pr_info("%s: Device config - version=0x%x, features=0x%llx\n",
            SST_DRIVER_NAME, sdev->config.version, sdev->config.features);
...
int sst_probe(struct virtio_device *vdev)
{
    ...
    minor = ida_simple_get(&sst_minor_ida, SST_MINOR_BASE,
                          SST_MINOR_BASE + SST_MAX_DEVICES, GFP_KERNEL);
    if (minor < 0) {
        ret = minor;
    ...
    /* Create device node */
    sdev->dev = device_create(sst_class, &vdev->dev, sdev->devt, sdev,
                             "%s%d", SST_DEVICE_NAME, minor);
    if (IS_ERR(sdev->dev)) {
...
void sst_remove(struct virtio_device *vdev)
{
    ...
    pr_info("%s: Statistics - Requests: %lld, Responses: %lld, Events: %lld, Errors: %lld\n",
            SST_DRIVER_NAME,
            atomic64_read(&sdev->total_requests),
            atomic64_read(&sdev->total_responses),
            atomic64_read(&sdev->total_events),
            atomic64_read(&sdev->total_errors));
...
static int __init sst_init(void)
{
    ...
    ret = alloc_chrdev_region(&sst_devt_base, SST_MINOR_BASE,
                             SST_MAX_DEVICES, SST_DRIVER_NAME);
    if (ret) {
