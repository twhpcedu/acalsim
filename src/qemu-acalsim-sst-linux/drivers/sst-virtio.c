...
int sst_init_vqs(struct sst_virt_device* sdev) {
	...
	struct virtqueue_info vqs_info[SST_VQ_MAX] = {
	    {"request", sst_response_done}, {"response", NULL}, {"event", sst_event_done}};
	int ret;

	/* Find and initialize virtqueues using new API */
	ret = virtio_find_vqs(sdev->vdev, SST_VQ_MAX, vqs, vqs_info, NULL);
	if (ret) {
...
int sst_probe(struct virtio_device* vdev) {
	...
	minor = ida_alloc_range(&sst_minor_ida, SST_MINOR_BASE, SST_MINOR_BASE + SST_MAX_DEVICES - 1, GFP_KERNEL);
	if (minor < 0) {
		ret = minor;
...
void sst_remove(struct virtio_device* vdev) {
	...
	/* Free minor number */
	ida_free(&sst_minor_ida, sdev->minor);
...
static int __init sst_init(void) {
	...
	/* Create device class */
	sst_class = class_create(SST_DRIVER_NAME);
	if (IS_ERR(sst_class)) {
