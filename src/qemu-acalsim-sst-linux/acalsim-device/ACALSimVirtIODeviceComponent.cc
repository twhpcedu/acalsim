...
ACALSimVirtIODeviceComponent::ACALSimVirtIODeviceComponent(SST::ComponentId_t id, SST::Params& params)
	...
	clock_handler_ =
	    new SST::Clock::Handler<ACALSimVirtIODeviceComponent>(this, &ACALSimVirtIODeviceComponent::clockTick);
...
ACALSimVirtIODeviceComponent::~ACALSimVirtIODeviceComponent() { cleanupSocket(); }

void ACALSimVirtIODeviceComponent::setup() { out_.verbose(CALL_INFO, 1, 0, "Setup phase\n"); }
...
bool ACALSimVirtIODeviceComponent::clockTick(SST::Cycle_t cycle) {
	...
	if (!client_connected_) { checkForConnections(); }

	// Process incoming data
	if (client_connected_) { handleSocketData(); }
...
void ACALSimVirtIODeviceComponent::initSocket() {
	...
	if (server_fd_ < 0) { out_.fatal(CALL_INFO, -1, "Failed to create socket: %s\n", strerror(errno)); }
	...
	if (listen(server_fd_, 1) < 0) { out_.fatal(CALL_INFO, -1, "Failed to listen on socket: %s\n", strerror(errno)); }
...
void ACALSimVirtIODeviceComponent::sendResponse(const uint8_t* data, size_t len) {
	if (!client_connected_ || client_fd_ < 0) { return; }
