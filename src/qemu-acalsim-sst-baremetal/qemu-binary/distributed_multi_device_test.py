...
if my_rank == 0:
    ...
    qemu.addParams({
        ...
        "qemu_args": [
            ...
            "-device", f"acalsim-echo,addr={hex(echo_device1_base)},size={device_size}",
            "-device", f"acalsim-compute,addr={hex(compute_device_base)},size={device_size}",
            "-device", f"acalsim-mmio,addr={hex(mmio_device_base)},size={device_size},irq=1",
            "-device", f"acalsim-echo,addr={hex(echo_device2_base)},size={device_size}"
        ],
...
if my_rank == 1:
    ...
    peer_link.connect(
        (echo_dev1, "peer_port", "1ns"),
        (compute_dev, "peer_port", "1ns")
    )
    print(f"[Rank {my_rank}] Peer link configured between Echo1 and Compute (efficient local communication)")
