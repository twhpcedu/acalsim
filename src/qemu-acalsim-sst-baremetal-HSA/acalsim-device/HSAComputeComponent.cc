...
#include "HSAComputeComponent.hh"
...
HSAComputeComponent::HSAComputeComponent(SST::ComponentId_t id, SST::Params& params) :
    ...
    aql_port_ = configureLink("aql_port",
        new SST::Event::Handler<HSAComputeComponent>(this, &HSAComputeComponent::handleAQLPacket));
    if (!aql_port_) {
    ...
    doorbell_port_ = configureLink("doorbell_port",
        new SST::Event::Handler<HSAComputeComponent>(this, &HSAComputeComponent::handleDoorbellRing));
    ...
    registerClock(clock_freq,
        new SST::Clock::Handler<HSAComputeComponent>(this, &HSAComputeComponent::clockTick));

    output_.verbose(CALL_INFO, 1, 0,
        "HSA Compute Component initialized: queue_depth=%u, cycles_per_workitem=%lu\n",
...
void HSAComputeComponent::setup() {
    output_.verbose(CALL_INFO, 1, 0, "HSA Compute setup complete\n");
}

void HSAComputeComponent::finish() {
    output_.verbose(CALL_INFO, 1, 0,
        "HSA Compute finish: Kernels=%lu, Total Workitems=%lu\n",
        kernels_executed_, total_workitems_);
}
...
bool HSAComputeComponent::clockTick(SST::Cycle_t currentCycle) {
    ...
    if (current_kernel_ && currentCycle >= current_kernel_->completion_time) {
        ...
        output_.verbose(CALL_INFO, 1, 0,
            "Kernel completed: dispatch=%u, latency=%lu cycles, workitems=%lu\n",
    ...
    if (!current_kernel_ && !aql_queue_.empty()) {
        ...
        current_kernel_->completion_time = currentCycle +
            calculateKernelLatency(current_kernel_->packet);

        output_.verbose(CALL_INFO, 2, 0,
            "Starting kernel: dispatch=%u, estimated_latency=%lu cycles\n",
...
void HSAComputeComponent::handleAQLPacket(SST::Event* ev) {
    ...
    if (!packet) {
        output_.fatal(CALL_INFO, -1, "Error: Received non-AQL event on aql_port\n");
    }

    output_.verbose(CALL_INFO, 2, 0,
        "Received AQL packet: type=%u, dispatch=%u, signal=0x%lx, workitems=%lu\n",
        packet->getPacketType(), packet->dispatch_id,
        packet->completion_signal, packet->getTotalWorkitems());

    // Check queue capacity
    if (aql_queue_.size() >= queue_depth_) {
        output_.output(CALL_INFO,
            "Warning: AQL queue full (depth=%u), dropping packet\n", queue_depth_);
...
void HSAComputeComponent::handleDoorbellRing(SST::Event* ev) {
    ...
    if (doorbell) {
        output_.verbose(CALL_INFO, 3, 0,
            "Doorbell ring received: queue=%u, value=%lu\n",
            doorbell->queue_id, doorbell->doorbell_value);
    }
    delete ev;
}
...
uint64_t HSAComputeComponent::calculateKernelLatency(HSAAQLPacketEvent* packet) {
    ...
    output_.verbose(CALL_INFO, 3, 0,
        "Kernel latency breakdown: launch=%lu, compute=%lu, memory=%lu, total=%lu\n",
...
void HSAComputeComponent::sendCompletionSignal(uint64_t signal_handle, int64_t value, uint32_t dispatch_id) {
    ...
    output_.verbose(CALL_INFO, 2, 0,
        "Sent completion signal: handle=0x%lx, value=%ld, dispatch=%u\n",
        signal_handle, value, dispatch_id);
}
