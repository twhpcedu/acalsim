...
if my_rank == 0:
	...
	qemu.addParams({
	    ...
	    "qemu_args": [
	        "-M", "virt", "-cpu", "rv64", "-m", "128M", "-nographic", "-bios", bios_image,
	        "-kernel", kernel_image, "-device",
	        f"acalsim-hsa-host,addr={hex(hsa_host_base)},size={device_size}", "-device",
	        f"acalsim-hsa-compute,addr={hex(hsa_compute1_base)},size={device_size}", "-device",
	        f"acalsim-hsa-compute,addr={hex(hsa_compute2_base)},size={device_size}"
	    ],
...
link_hsa_host.connect((qemu, "device_port_0", "1ns"), (hsa_host, "cpu_port", "1ns"))
