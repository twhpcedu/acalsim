...
class DeviceSpec:
	...
	def __init__(
	    self, name: str, base_addr: int, size: int, component: str, params: Dict[str, Any] = None
	):
		self.name = name
...
""".format(len(self.devices))
	...
	def _generate_devices(self) -> str:
		...
		for i, dev in enumerate(self.devices):
			...
			device_params = {"clock": "1GHz", "verbose": 2, **dev.params}
...
def load_devices_from_json(json_file: str) -> List[DeviceSpec]:
	...
	for dev_data in data.get("devices", []):
		device = DeviceSpec(
		    name=dev_data["name"],
		    base_addr=int(dev_data["base_addr"], 16)
		    if isinstance(dev_data["base_addr"], str) else dev_data["base_addr"],
...
def generate_default_devices(
    num_devices: int, base_addr: int = 0x10200000, spacing: int = 0x100000
) -> List[DeviceSpec]:
...
JSON Format:
{
  "devices": [
	...
	parser.add_argument(
	    "--binary", help="Path to RISC-V binary (default: riscv-programs/hello.elf)"
	)
